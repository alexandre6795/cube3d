...
/*   Updated: 2024/01/05 15:24:23 by akastler         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (cube->player->ray[ray_nbr].hitted == 1 || cube->player->ray[ray_nbr].hitted == 3)
	{
		wall->value = (((int)(cube->player->ray[ray_nbr].x) % wall->texture->height)
				+ ((int)wall->current * wall->texture->height)) * 4;
	}
	else
	{
		wall->value = (((int)(cube->player->ray[ray_nbr].y) % wall->texture->height)
				+ ((int)wall->current * wall->texture->height)) * 4;
	}
	wall->color = rgba(wall->texture->pixels[wall->value], wall->texture->pixels[wall->value + 1], wall->texture->pixels[wall->value + 2], wall->texture->pixels[wall->value + 3]);
