...
/*   Updated: 2024/01/23 11:11:52 by aherrman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	*wall_collision_vert(t_cube *cube, t_ray *ray)
{
		...
		&& rc.actxy[1] < cube->map->size_x * 64)
	{
		if (cube->map->map[(int)rc.actxy[1] / 64][(int)rc.actxy[0] / 64] == '1'
			|| cube->map->map[(int)rc.actxy[1] / 64][(int)rc.actxy[0]
			/ 64] == 'D' || cube->map->map[(int)rc.actxy[1]
			/ 64][(int)rc.actxy[0] / 64] == 'O')
		{
			ray->block_type[0] = cube->map->map[(int)rc.actxy[1]
				/ 64][(int)rc.actxy[0] / 64];
			break ;
		}
...
float	*wall_collision_hori(t_cube *cube, t_ray *ray)
{
		...
		&& rc.actxy[1] < cube->map->size_x * 64)
	{
		if (cube->map->map[(int)rc.actxy[1] / 64][(int)rc.actxy[0] / 64] == '1'
			...
			/ 64][(int)rc.actxy[0] / 64] == 'O')
		{
			ray->block_type[1] = cube->map->map[(int)rc.actxy[1]
				/ 64][(int)rc.actxy[0] / 64];
			break ;
		}
		rc.actxy[0] += rc.incrementxy[0];
